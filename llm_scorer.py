...
# Топ-N ВНУТРИ КАЖДОГО блока опыта (см. AUDIENCE_BLOCKS) по score - не общий
# потолок пачки, как было раньше (AUDIENCE_BATCH_CAP). Остальные кандидаты
# просто остаются непомеченными в журнале «Аудитория» и уйдут следующим
# прогоном - бэклог не теряется, просто не спешит.
AUDIENCE_BLOCK_TOP = 3
...
SYSTEM_PROMPT = f"""Ты - строгий ассистент по разбору вакансий продакт-менеджера.
Оцениваешь вакансию саму по себе: настоящая ли это продуктовая IT-роль и
насколько она хороша по существу - НЕ под конкретного кандидата. Работай
ТОЛЬКО с фактами из текста вакансии. ЗАПРЕЩЕНО: выдумывать данные,
приукрашивать, добавлять то, чего нет в тексте. Если поля нет в тексте -
ставь "не указано". Вердикт - сухой и конкретный, без похвал и воды,
...
{{
  ...
  "score": "целое от 0 до 10 строго по шкале ниже",
  "verdict": "1 сухое предложение на русском языке почему такая оценка, максимум 25 слов"
}}
Поле experience определяй по обязательным требованиям к кандидату.
Ориентируйся на минимальный требуемый стаж: «без опыта», «стажировка»,
«студентам» - «Без опыта»; от 1 года, 1-2 года или от 2 лет -
«1-3 года»; от 3, 4 или 5 лет - «3-6 лет»; от 6 лет и выше - «6+ лет».
Не учитывай опыт, указанный только как преимущество или пожелание.
Ставь «не указано» только тогда, когда в тексте нет указаний на
обязательный стаж. Это поле не влияет на оценку - оно используется
только для группировки.
Если для работы нужно физически находиться за пределами РФ (офис или гибрид
за границей) - занижай: аудитория ищет роли в России или с полной удалёнкой.
Полная удалённая работа из другой страны - не повод занижать.
Оценивай в строгом порядке:
1. Это одна конкретная вакансия или нет.
2. Кого именно нанимают: специалиста по управлению продуктом либо
специалиста другой профессии.
3. Только после определения роли выставляй оценку её качества.

Целевая роль - Product Manager, Product Owner или аналогичная должность,
в которой человек отвечает за развитие продукта, продуктовые решения,
приоритеты или результаты продукта.

Название должности само по себе не является достаточным доказательством.
Вакансия с названием Product Manager может быть нецелевой, если основные
обязанности сводятся к координации сроков, ведению проектов, продажам,
сопровождению клиентов или администрированию задач.

Не считай отдельные ключевые слова. Определяй роль по смыслу основных
обязанностей. Упоминание аналитики, разработки, дизайна или маркетинга
как участников команды не снижает оценку продакт-менеджера.

Шкала оценки:
- 1-3 - вакансия существует, но нанимают не специалиста по управлению
продуктом;
- 4 - роль неоднозначная: смешаны Product и Project/Delivery/
Account-функции либо недостаточно информации, чтобы подтвердить
продуктовую ответственность;
- 5 - целевая роль подтверждена, но продукт, зона ответственности или
задачи описаны поверхностно;
- 6-7 - полноценная продуктовая роль с конкретным продуктом и понятными
задачами; присутствует ответственность за приоритеты, метрики,
пользователей или развитие продукта;
- 8-9 - сильная продуктовая роль с ясной ответственностью за результат,
самостоятельностью в решениях и несколькими содержательными
элементами: исследования, гипотезы, метрики, приоритизация, roadmap,
эксперименты;
- 10 - исключительно полная и конкретная роль с ответственностью за
продуктовый результат от исследования проблемы до оценки эффекта.
Используй редко.

Если is_vacancy=false (реклама, подборка, инфопост), оценка не важна -
ставь 0.

Наличие крупной компании, высокой зарплаты или удалённой работы не может
поднять нецелевую роль выше 3.

Начинай вердикт с одного из вариантов:
- «Целевая роль: …»
- «Нецелевая роль: …»
- «Неоднозначная роль: …»
После двоеточия назови главный факт, определивший оценку."""
...
# ==========================================================================
# 4 блока рассылки, в порядке вывода: (значение "Опыт (треб.)", показывать ли
# в счётчике шапки, заголовок секции). Само значение блока ('Без опыта',
# '1-3 года', '3-6 лет') уже написано ровно так, как нужно в строке счётчика
# шапки - отдельной подписи не заводим, используем ключ напрямую.
# '6+ лет' сюда сознательно не включён - вакансия с таким опытом не совпадёт
# ни с одним ключом блока и просто не сгруппируется, отдельного "если 6+,
# то пропустить" кода не нужно (см. build_audience_candidates).
AUDIENCE_BLOCKS = [
    ('Без опыта', True, 'Без опыта / стажировка'),
    ('1-3 года', True, '1-3 года'),
...
# несёт информации, "не указано" не стоит печатать в позиции, где рядом уже
# печатается 'не указана' про зарплату - визуальный шум.
_AUDIENCE_FORMAT_LABELS = {'Удалёнка': 'удалённо', 'Гибрид': 'гибрид'}

def _audience_source_tag(src):
    """'Хабр' -> [Хабр]; f'Telegram/{канал}' (или что угодно ещё) -> [TG]."""
    return '[Хабр]' if src == 'Хабр' else '[TG]'
...
def build_audience_candidates(ws, already_sent):
    ...
    Возвращает {значение_опыта: [кандидаты...]} - словарь с ровно 4 ключами
    AUDIENCE_BLOCKS, каждый список уже отсортирован и обрезан до топ-3."""
    rows = with_retry(lambda: ws.get_all_records(), what="чтение листа «Вакансии» для аудитории")
    by_block = {key: [] for key, _, _ in AUDIENCE_BLOCKS}
    for row in rows:
        sid = str(row.get('source_id', '')).strip()
        ...
        by_block[exp].append({
            ...
            'salary': str(row.get('ЗП вилка') or 'не указано'),
            'format': str(row.get('Формат') or ''),
            'location': str(row.get('Локация') or ''),
            'src': str(row.get('Источник') or ''),
            'url': str(row.get('Ссылка') or ''),
            'score': score,
        })
    for key in by_block:
        by_block[key].sort(key=lambda v: v['score'], reverse=True)
        by_block[key] = by_block[key][:AUDIENCE_BLOCK_TOP]
    return by_block
...
def _audience_item_lines(n, v):
    ...
    salary = v['salary'] if v['salary'] and v['salary'] != 'не указано' else 'не указана'
    loc_parts = []
    if v['location'] and v['location'] != 'не указано':
        loc_parts.append(v['location'])
    fmt_label = _AUDIENCE_FORMAT_LABELS.get(v['format'])
    # не дублируем формат, если словесная форма уже встречается в тексте
    # локации (например "Варшава (гибрид, 2 дня офис, 3 удалённо)") -
    # регистронезависимое вхождение подстроки.
    if fmt_label and fmt_label.lower() not in (v['location'] or '').lower():
        loc_parts.append(fmt_label)
    tail = ', '.join(loc_parts)
    second = html.escape(salary) + (f' · {html.escape(tail)}' if tail else '')
    return [head, f'   {second}']
...
def notify_audience(gc, ss_raw, ws_vacancies):
    ...
    by_block = build_audience_candidates(ws_vacancies, sent_ids)
    all_candidates = [v for items in by_block.values() for v in items]
    if not all_candidates:
        return sent_ids   # во всех 4 блоках пусто - подписчикам не шлём ничего
