...
{{
  ...
  "score": "целое 0-10, насколько это качественная и настоящая продуктовая роль (10=идеально, 0=совсем нет)",
  "verdict": "1 сухое предложение на русском языке почему такая оценка, максимум 25 слов"
}}
Вакансии за пределами РФ без возможности удалённой работы - занижай: аудитория
ищет роли в России или с удалёнкой.
...
Внутри настоящих продуктовых ролей оценивай выше те, где понятно, каким продуктом
и чем именно предстоит заниматься, есть конкретика по задачам и указаны условия;
ниже - где описание общее, требования размытые, а о продукте почти ничего не
сказано.
...
# ==========================================================================
# 4 блока рассылки, в порядке вывода: (значение "Опыт (треб.)", подпись для
# счётчика в шапке ИЛИ None если блок в шапку не входит, заголовок секции).
...
AUDIENCE_BLOCKS = [
    ('Без опыта', 'без опыта', 'Без опыта / стажировка'),
    ('1-3 года', '1-3 года', '1-3 года'),
    ('3-6 лет', '3-6 лет', '3-6 лет'),
    ('не указано', None, 'Опыт не указан'),
]
...
def build_audience_messages(by_block):
    ...
    потока: заголовок со счётчиком по 3 основным блокам (всегда, даже при
    нуле), затем секции непустых блоков (включая "Опыт не указан", если
    непуст) - пустые блоки в теле не показываем, нумерация внутри каждого
    блока заново с 1. Разбиение по границам строк - механизм от прежней
    построчной рассылки; при максимум 4×3=12 позициях почти никогда не
    сработает, остаётся защитным запасом на аномально длинные тексты."""
    counters = ' · '.join(
        f'{label} {len(by_block[key])}'
        for key, label, _ in AUDIENCE_BLOCKS if label is not None)
    lines = [f'🎯 Product manager: {counters}']
    for key, _, header in AUDIENCE_BLOCKS:
        items = by_block[key]
        if not items:
            continue
        lines.append('')
